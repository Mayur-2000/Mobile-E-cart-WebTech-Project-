<?php
	...
	function select4Devs($conn){
		$row = array();
		$query = "SELECT mod_num, image FROM devices ORDER BY mod_num DESC";
	...
	function getDevBymdnm($conn, $mod_num){
		$query = "SELECT dev_name, company, price FROM devices WHERE mod_num = '$mod_num'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getdevprice($mod_num){
		$conn = db_connect();
		$query = "SELECT price FROM devices WHERE mod_num = '$mod_num'";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getAll($conn){
		$query = "SELECT * from devices ORDER BY mod_num DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
